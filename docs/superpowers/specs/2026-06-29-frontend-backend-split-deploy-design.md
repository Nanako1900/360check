...
**安全头 / CSP / SPA 回退 / 缓存**：从 `web-admin/deploy/nginx.conf` 搬到 EdgeOne Pages 配置（`_headers` / `_redirects` 或 EdgeOne 规则引擎 —— **确切机制待实现时查 EdgeOne 官方文档**，见 §6）：
...
## 6. 待实现时确认（Open Items）
1. **EdgeOne Pages 头/重定向确切机制**：`_headers`/`_redirects`（CF 兼容）vs EdgeOne 规则引擎 / `edgeone.json`。实现时查 EdgeOne 官方文档（可用 `docsdl` 拓文档 / `coolify-ops` 文档流程）。配置**值**已在 §4.3 给定，仅落地形式待定。
