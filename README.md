...
- **头 / CSP / SPA 回退**：[`web-admin/public/_headers`](web-admin/public/_headers) · [`_redirects`](web-admin/public/_redirects)（Vite 复制到 `dist/`）。⚠️ 把 `_headers` 里 CSP `connect-src` 的 `api.example.com` 替换为真实 API 源。
