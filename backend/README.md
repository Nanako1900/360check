...
## Coolify / Docker Compose 部署

整栈定义在仓库根 [`../docker-compose.yaml`](../docker-compose.yaml)（Coolify「Docker Compose」指向它）；`api`/`worker` 为内部服务，仅 `web` 暴露域名并同源反代 `/api→api:8080`。后端为此提供三个 `c5-api` 子命令（同一镜像，distroless）：

| 子命令 | 用途 | 需要的环境变量 |
|---|---|---|
| `c5-api migrate` | 跑 golang-migrate up（按 version 幂等，持 advisory lock）。compose `migrate` 一次性容器调用。 | **仅** `C5_DB_DSN` |
| `c5-api create-admin` | 用 `C5_BOOTSTRAP_ADMIN_PASSWORD`（bcrypt cost 12）设首位管理员。**部署后手动跑一次**，随后删该变量。 | 完整配置（DB/Redis/JWT） |
| `c5-api healthcheck` | GET `127.0.0.1:$C5_SERVER_PORT/livez`，2xx→0 否则 1。供 Dockerfile `HEALTHCHECK`（distroless 无 curl）。 | `C5_SERVER_PORT`（默认 8080） |

- `migrate` 只读 `C5_DB_DSN`（不经全量 `config.Load`），故一次性迁移容器无需 Redis/JWT。
- `Dockerfile.api` 内置 `HEALTHCHECK ... CMD ["/c5-api","healthcheck"]`；compose 据此做 `api: service_healthy` 门控。
- 本地整栈验证：仓库根 `cp .env.example .env && docker compose up --build`（占位 magic vars；COS 留空→media mock）。
- 详见根 [`README.md`](../README.md) 的「Coolify 部署」段（magic vars、Build Variable、出网与地图白名单、create-admin 一次性步骤）。
